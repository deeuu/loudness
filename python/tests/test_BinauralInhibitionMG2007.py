    ...
    def process(self, specificLoudness):
        ...
        for ear in range(2):
            self.smoothSpecificLoudness[:,ear] = \
                np.convolve(self.fullGaussian, specificLoudness[:,ear])[nFilters-1:-nFilters+1]
    
        #Gain computation  
        self.smoothSpecificLoudness[self.smoothSpecificLoudness < 1e-12] = 1e-12
        inhibLeft = 2.0 / (1 + (1.0/np.cosh(self.smoothSpecificLoudness[:,1] /\
                self.smoothSpecificLoudness[:,0]))**1.5978);
        inhibRight = 2.0 / (1 + (1.0/np.cosh(self.smoothSpecificLoudness[:,0] /\
                self.smoothSpecificLoudness[:,1]))**1.5978);
