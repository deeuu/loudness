...
namespace loudness{
    ...
    Real soneToPhonDIN456311991 (Real sone, bool isPolyApprox)
    {
        if (isPolyApprox)
        {
            ...
            else if (s >= -2.8521714453054199) // >= 15.1 dB
            {
                return (-0.093820769025946174 * s5 - 0.57927163031988804 * s4 -
                        0.94119115719666124 * s3 + 1.3728561867856046 * s2 +
                        13.048552615771374 * s + 39.79975651257859);
            }
            else if (s >= -7.198248105954967) // > 5 dB
            {
                return (0.51323454877808472 * s2 +
                        7.458314867983173 * s + 32.211834631396748);
            }
            else
            {
                return 5.0;
